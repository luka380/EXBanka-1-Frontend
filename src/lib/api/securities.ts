...
export async function getStocks(filters: StockFilters = {}): Promise<StockListResponse> {
  const { data } = await apiClient.get<StockListResponse>('/securities/stocks', {
    params: filters,
  })
...
export async function getStock(id: number): Promise<Stock> {
  const { data } = await apiClient.get<Stock>(`/securities/stocks/${id}`)
...
export async function getStockHistory(
  ...
  const { data } = await apiClient.get<PriceHistoryResponse>(`/securities/stocks/${id}/history`, {
    params: filters,
  })
...
export async function getFutures(filters: FuturesFilters = {}): Promise<FuturesListResponse> {
  const { data } = await apiClient.get<FuturesListResponse>('/securities/futures', {
    params: filters,
  })
...
export async function getFuture(id: number): Promise<FuturesContract> {
  const { data } = await apiClient.get<FuturesContract>(`/securities/futures/${id}`)
  return flattenListing(data)
}
...
export async function getFutureHistory(
  ...
  const { data } = await apiClient.get<PriceHistoryResponse>(`/securities/futures/${id}/history`, {
    params: filters,
  })
  return { ...data, history: data.history ?? [] }
}
...
export async function getForexPairs(filters: ForexFilters = {}): Promise<ForexListResponse> {
  const { data } = await apiClient.get<ForexListResponse>('/securities/forex', {
    params: filters,
  })
...
export async function getForexPair(id: number): Promise<ForexPair> {
  const { data } = await apiClient.get<ForexPair>(`/securities/forex/${id}`)
  return flattenListing(data)
}
...
export async function getForexHistory(
  ...
  const { data } = await apiClient.get<PriceHistoryResponse>(`/securities/forex/${id}/history`, {
    params: filters,
  })
  return { ...data, history: data.history ?? [] }
}
...
export async function getOptions(filters: OptionsFilters): Promise<OptionsListResponse> {
  const { data } = await apiClient.get<OptionsListResponse>('/securities/options', {
    params: filters,
  })
...
export async function getOption(id: number): Promise<Option> {
  const { data } = await apiClient.get<Option>(`/securities/options/${id}`)
  return flattenListing(data)
}
...
export async function createOptionOrder(
  ...
  const { data } = await apiClient.post<Order>(`/options/${optionId}/orders`, payload)
  return data
}

export async function exerciseOption(optionId: number): Promise<ExerciseOptionResponse> {
  const { data } = await apiClient.post<ExerciseOptionResponse>(`/options/${optionId}/exercise`)
  return data
}
